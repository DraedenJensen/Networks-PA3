...
if args.daemons:
    ...
    if len(client.containers.list()) == 0:
        ...
        for container in client.containers.list():
            ...
            print("*****************************************************************************************************************************************")
            subprocess.run(["docker", "exec", container.name, "-it", "apt -y install curl"])
            subprocess.run(["docker", "exec", container.name, "-it", "apt -y install gnupg"])
            subprocess.run(["docker", "exec", container.name, "-it", "curl -s https://deb.frrouting.org/frr/keys.gpg | tee /usr/share/keyrings/frrouting.gpg > /dev/null"])
            subprocess.run(["docker", "exec", container.name, "-it", "apt install lsb-release"])
            subprocess.run(["docker", "exec", container.name, "-it", "FRRVER=\"frr-stable\""])
            subprocess.run(["docker", "exec", container.name, "-it", "echo deb '[signed-by=/usr/share/keyrings/frrouting.gpg]’ https://deb.frrouting.org/frr $(lsb_release -s -c) $FRRVER | tee -a /etc/apt/sources.list.d/frr.list"])
            subprocess.run(["docker", "exec", container.name, "-it", "apt update && apt -y install frr frr-pythontools"])
